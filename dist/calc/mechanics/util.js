...
function checkLuckyBall(gen, source, target) {
    var blocked = target.hasAbility('Clear Body', 'White Smoke', 'Hyper Cutter', 'Full Metal Body') ||
        (gen.num >= 8 && target.hasAbility('Inner Focus', 'Own Tempo', 'Oblivious', 'Scrappy')) ||
        target.hasItem('Clear Amulet');
    if (source.hasAbility('Lucky Ball') && source.abilityOn && !blocked) {
        if (target.hasAbility('Contrary')) {
            target.boosts.def = Math.min(6, target.boosts.def + 1);
        }
        else if (target.hasAbility('Simple')) {
            target.boosts.def = Math.max(-6, target.boosts.def - 2);
        }
        else {
...
exports.checkLuckyBall = checkLuckyBall;
